...
- [第一种同余乘法](https://vjudge.net/solution/19733874)
...
struct Mod
{
	...
	ll mul(ll a, ll b) const //无循环快速计算同余乘法，根据a*b是否爆ll替换a*b%m，需要a<M且b<M，可以调用时手动取模
	{
		ll c = a / SM, d = b / SM;
		a %= SM, b %= SM;
		ll e = SM * SM - M, f = add(qadd(a * d, b * c), c * d / SM * e);
		return qadd(qadd(a * b, f % SM * SM), add(f / SM * e, c * d % SM * e));
	}
	*/
