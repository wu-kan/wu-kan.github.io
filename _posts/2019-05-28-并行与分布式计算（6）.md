...
删去设备同步的调用之后，会使得分配的cuda内存过快被释放，后续核函数对其访问会导致段错误。
...
int main(void)
{
    ...
    */
	...
	CUDA_CHECK(cudaMemcpy(dA, hA, N, cudaMemcpyHostToDevice));
	CUDA_CHECK(cudaMemcpy(dB, hB, N, cudaMemcpyHostToDevice));
	...
	CUDA_CHECK(cudaMemcpy(hC, dC, N, cudaMemcpyDeviceToHost));
	CUDA_CHECK(cudaFree(hA));
	CUDA_CHECK(cudaFree(hB));
	CUDA_CHECK(cudaFree(hC));
						 ...
						 double h2, int N)
{
	...
	for (int index = threadIdx.x + blockIdx.x * blockDim.x + 1; index + 1 + N < N * N; index += blockDim.x * gridDim.x)
	{
		int i1 = index - 1, i2 = index + 1, i3 = index - N, i4 = index + N;
		phi[index] = 0.25 * (phiPrev[i1] + phiPrev[i2] +
							 phiPrev[i3] + phiPrev[i4] -
							 h2 * source[index]);
	}
...
int main()
{
	...
	while (diff > tolerance && iterations < MAX_ITERATIONS)
	{
		...
		//// Add routines here
		sweepGPU(phiPrev_d, phi_d, source, h * h, N);
		sweepGPU(phi_d, phiPrev_d, source, h * h, N);
