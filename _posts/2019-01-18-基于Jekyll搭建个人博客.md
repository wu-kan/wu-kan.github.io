...
## 定制
我已经把大部分博客用到的插件都封装起来了，拿走的流程都差不多：
...
已经将相关代码封装，将我博客代码仓库下`_includes\live2d-widget.html`加入你自己的`_includes`，在`<body>`标签内加入如下代码。
```
...
已经将相关代码封装，将我博客代码仓库下`_includes\katex.html`加入你自己的`_includes`，在`<body>`标签内加入如下代码。
```
{% include katex.html %}
```
随后在你的`_config.yml`内加入如下配置项：
...
```
### mermaid流程图
已经将相关代码封装，有空探索下Markdown扩展怎么写。

### valine评论系统
详见[官方手册](https://valine.js.org/)，说得很清楚了。
...
valine: # depend av-min.js #Leancloud
  ...
  var:
    - key: appId
      value: '9hABRddSuEkTgqLrt1VSK5B1-gzGzoHsz'

    - key: appKey
      value: 'NJ7RwmgrxsF7KDzlqU7YewlL'
