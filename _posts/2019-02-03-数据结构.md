...
```
## 动态开点线段树
[使用示例](https://vjudge.net/solution/18435320)，支持区间线性变换、区间查询（最大值最小值区间和）。
...
struct SegmentTree
{
	...
	Val up(const Val &lc, const Val &rc) { return {lc.l, rc.r, min(lc.min, rc.min), max(lc.max, rc.max), lc.sum + rc.sum}; }
	void down(int rt)
	{
