...
```
## 动态开点线段树
[使用示例](https://vjudge.net/solution/18403952)，支持区间先乘再加、区间查询（最大值最小值区间和）。
...
struct SegmentTree
{
	...
	struct Node : Val
	{
		...
		void upd(ll m, ll a) { mul *= m, add = add * m + a, Val::upd(m, a); }
	};
	vector<Node> v;
	...
	void down(int rt)
	{
		...
		v[v[rt].lc].upd(v[rt].mul, v[rt].add);
		v[v[rt].rc].upd(v[rt].mul, v[rt].add);
		v[rt].mul = 1;
		v[rt].add = 0;
	}
