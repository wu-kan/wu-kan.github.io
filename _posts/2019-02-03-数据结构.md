...
```
## 线段树
[使用示例](https://vjudge.net/solution/17060787)，支持区间先乘再加、区间查询（最大值最小值区间和）
```c++
struct SegmentTree
{
	struct Node
	{
		...
		void down(Node &lc, Node &rc) { lc.set(mul, add), rc.set(mul, add), mul = 1, add = 0; }
	};
	vector<Node> v;
	...
	void set(int l, int r, ll mul, ll add, int rt = 1)
	{
		if (l <= v[rt].l && v[rt].r <= r)
			return v[rt].set(mul, add);
		v[rt].down(v[rt << 1], v[rt << 1 | 1]);
		int m = v[rt].l + v[rt].r >> 1;
		if (m >= r)
			set(l, r, mul, add, rt << 1);
		else if (m < l)
			set(l, r, mul, add, rt << 1 | 1);
		else
			set(l, m, mul, add, rt << 1), set(m + 1, r, mul, add, rt << 1 | 1);
