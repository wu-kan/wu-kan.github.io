...
```
## 单调队列和单调栈
[使用示例](https://vjudge.net/solution/16972636)
```cpp
...
```
## ST表
[使用示例](https://cn.vjudge.net/solution/14470893)
...
```
## 动态开点线段树
[使用示例](https://vjudge.net/solution/18400672)，支持区间先乘再加、区间查询（最大值最小值区间和）。
...
struct SegmentTree
{
	...
	void build(int l, int r) { lc.push_back(NPOS), rc.push_back(NPOS), v.emplace_back(l, r); }
	void down(int rt)
	{
		int m = v[rt].l + v[rt].r >> 1;
		if (lc[rt] == NPOS)
			lc[rt] = v.size(), build(v[rt].l, m);
		if (rc[rt] == NPOS)
			rc[rt] = v.size(), build(m + 1, v[rt].r);
		v[lc[rt]].upd(v[rt].mul, v[rt].add);
		v[rc[rt]].upd(v[rt].mul, v[rt].add);
		v[rt].mul = 1;
		v[rt].add = 0;
	}
	void upd(int l, int r, ll mul, ll add, int rt = 0)
	{
		...
		down(rt);
		if (r <= v[lc[rt]].r)
			upd(l, r, mul, add, lc[rt]);
		else if (l >= v[rc[rt]].l)
			upd(l, r, mul, add, rc[rt]);
		else
			upd(l, v[lc[rt]].r, mul, add, lc[rt]), upd(v[rc[rt]].l, r, mul, add, rc[rt]);
		v[rt] = Node(v[lc[rt]], v[rc[rt]]);
	}
	Node ask(int l, int r, int rt = 0)
	{
		if (l <= v[rt].l && v[rt].r <= r)
			return v[rt];
		down(rt);
		if (r <= v[lc[rt]].r)
			return ask(l, r, lc[rt]);
		if (l >= v[rc[rt]].l)
			return ask(l, r, rc[rt]);
		return Node(ask(l, v[lc[rt]].r, lc[rt]), ask(v[rc[rt]].l, r, rc[rt]));
	}
};
```
## 无旋Treap
### 按子树大小分裂
[使用示例](https://vjudge.net/solution/16558390)
```cpp
struct FhqTreap
{
	struct Node
...
```
### 按值大小分裂
[使用示例](https://vjudge.net/solution/16500498)，即排序树。
```cpp
struct FhqTreap
{
	struct Node
	...
	void splitVal(int a, ll w, int &l, int &r) //按值将树划分，使得左子树上的值恰小于w
	{
		if (!a)
			...
			splitVal(v[a].ch[0], w, l, v[a].ch[0]), push_up(r = a);
		else
			splitVal(v[a].ch[1], w, v[a].ch[1], r), push_up(l = a);
	}
	void insert(ll x)
	{
		int a, b;
...
```
## 莫队
[使用示例](https://cn.vjudge.net/solution/14265915)
```cpp
struct Mo
{
	struct Query
