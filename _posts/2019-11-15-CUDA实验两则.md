...
> 完成cuda的“Hello world”程序，编译运行`grid(2, 4), block(8, 16)`，给出输出结果文件。
...
矩阵加法可以直接类比于向量加法来做；使用CUDA在nvidia显卡上面编程时，可以使用CUDA提供的Event进行程序计时。
...
可以看到，即使算上设备内存分配和拷贝的事件，GPU上的矩阵加法还是要快于CPU的。
