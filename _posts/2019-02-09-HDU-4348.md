...
struct SegmentTree
{
	struct Val
	{
		int l, r;
		ll sum;
		Val(int l, int r) : l(l), r(r), sum(a[r] - a[l - 1]) {}
	...
	struct Node : Val
	{
		int lc, rc;
		ll add;
		Node(Val v) : Val(v), lc(NPOS), rc(NPOS), add(0) {}
	};
	vector<Node> v;
	SegmentTree(int l, int r) : v{Val(l, r)} {}
	void down(int rt)
	{
		int m = (v[rt].l + v[rt].r) / 2;
		v.push_back(v[rt].lc != NPOS ? v[v[rt].lc] : Node(Val(v[rt].l, m)));
		v[rt].lc = v.size() - 1;
		v.push_back(v[rt].rc != NPOS ? v[v[rt].rc] : Node(Val(m + 1, v[rt].r)));
		v[rt].rc = v.size() - 1;
